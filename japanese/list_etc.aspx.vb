...
#End Region
    ...
    Private Sub GetTitle()
        ...
        If Request.QueryString("chk_etc") = "section" Then
            DS = WebNewsDB.GetDataSetBySQLMun("SELECT SectionNameJpn, lim_date from Sections WHERE SectionID = " + Request.QueryString("ID"))
            lbl_bar.Text = DS.Tables(0).Rows(0).Item("SectionNameJpn")
        Else
            DS = WebNewsDB.GetDataSetBySQLMun("SELECT LocalNameJpn, lim_date from Locals WHERE LocalID = " + Request.QueryString("ID"))
            lbl_bar.Text = DS.Tables(0).Rows(0).Item("LocalNameJpn")
        End If
    ...
    Private Sub BindData()
        ...
        Else
            str_lvl = "0"
        End If
        DS = WebNewsDB.GetArticlesByETC(Request.QueryString("ID"), Request.QueryString("chk_etc"), str_lvl, lim_date, 201) '일본어
