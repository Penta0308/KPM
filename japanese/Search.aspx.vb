...
#End Region
        ...
        If Not IsPostBack Then
            If Request("keyword") <> "" Then
                ...
                Select Case Request("media")
                    Case "news"
                        bindDataGrid(dgNews, pnlNews, pnlNewsMore, "", " Articles INNER Join  Media ON Articles.MediaID = Media.MediaID", "MediaNameJpn", "ArticleID", "Title", "JunSongDateTime", "Nayong1", "junsongdateTime desc", "Nayong2")
                        dgNews.PagerStyle.Visible = True
                    Case "journal"
                        bindDataGrid(dgJournal, pnlJournal, pnlJournalMore, "", "JournalsArticle INNER Join  Media ON JournalsArticle.JournalID = Media.MediaID", "MediaNameJpn", "JArticleID", "Title", "CAST(BalHengYear AS varchar(5)) + '년 ' + CAST(GwonHo AS varchar(5)) + '호' AS balheng", "Nayong1", "BalHengYear desc, gwonho desc", "Nayong2")
                    ...
                    Case Else
                        bindDataGrid(dgNews, pnlNews, pnlNewsMore, "5", " Articles INNER Join  Media ON Articles.MediaID = Media.MediaID", "MediaNameJpn", "ArticleID", "Title", "JunSongDateTime", "Nayong1", "junsongdateTime desc", "Nayong2")
                        bindDataGrid(dgJournal, pnlJournal, pnlJournalMore, "5", "JournalsArticle INNER Join  Media ON JournalsArticle.JournalID = Media.MediaID", "MediaNameJpn", "JArticleID", "Title", "CAST(BalHengYear AS varchar(5)) + '년 ' + CAST(GwonHo AS varchar(5)) + '호' AS balheng", "Nayong1", "BalHengYear desc, gwonho desc", "Nayong2")
    ...
    Sub bindDataGrid(ByVal DataGrid As WebControls.DataGrid, ByVal Panel As WebControls.Panel, ByVal PanelMore As WebControls.Panel, ByVal PageSize As String, ByVal Media As String, _
                ByVal Field_MediaNameJpn As String, ByVal Field_ID As String, ByVal Field_Title As String, _
        ...
        If Field_MediaNameJpn <> "" Then Query += " " + Field_MediaNameJpn + ","
